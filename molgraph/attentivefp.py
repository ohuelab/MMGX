...
class GATEConv(MessagePassing):
    ...
    def message(self, x_j: Tensor, x_i: Tensor, edge_attr: Tensor,
        ...
        # when edge is None, add all zeros
        if edge_attr is None:
            edge_attr = torch.zeros((x_j.shape[0], self.edge_dim)).to(x_j.device)
...
class AttentiveFP(torch.nn.Module):
    ...
    def forward(self, x: Tensor, edge_index: Tensor, edge_attr: Tensor, batch: Tensor) -> Tensor:
        ...
        return self.lin2(out), final_conv_acts, (att_mol_index, att_mol_mean)